...
impl Aig {
    ...
    pub fn migrate_logic(
        ...
        let flag = self.fanin_logic_cone(&[logic]);
    ...
    pub fn migrate_logic_ttt(
        &mut self,
        nodes: &Vec<(AigEdge, AigEdge)>,
        logic: AigEdge,
    ) -> AigEdge {
        let flag = self.fanin_logic_cone(&[logic]);
        let mut map = vec![None; self.num_nodes()];
        for (src, dest) in nodes {
            map[src.node_id()] = Some(AigEdge::new(dest.node_id(), src.compl() != dest.compl()));
        }
        for id in 0..self.num_nodes() {
            if flag[id] && map[id].is_none() {
                map[id] = Some(if self.nodes[id].is_and() {
